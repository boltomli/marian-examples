...
if [ ! -e "data/corpus.en" ] || [ ! -e "data/valid.en" ]
then
    echo "Please prepare corpus and validation data first, see README"
    exit 1
fi
...
then
    $MARIAN_TRAIN \
        ...
        --train-sets data/corpus.bpe.en data/corpus.bpe.zh \
        --max-length 100 \
        --vocabs model/vocab.enzh.yml model/vocab.enzh.yml \
        --mini-batch-fit -w 4000 --maxi-batch 1000 \
        --early-stopping 10 --cost-type=ce-mean-words \
        --valid-freq 5000 --save-freq 5000 --disp-freq 500 \
        --valid-metrics ce-mean-words perplexity translation \
        --valid-sets data/valid.bpe.en data/valid.bpe.zh \
...
do
    cat data/$prefix.bpe.en \
        | $MARIAN_DECODER -c model/model.npz.decoder.yml -m model/model.iter$ITER.npz -d $GPUS -b 12 -n -w 4000 \
        | sed 's/\@\@ //g' \
        | ../tools/moses-scripts/scripts/recaser/detruecase.perl \
        | ../tools/moses-scripts/scripts/tokenizer/detokenizer.perl -l zh \
        > data/$prefix.zh.output
done
